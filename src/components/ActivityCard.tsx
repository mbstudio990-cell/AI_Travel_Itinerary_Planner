...
import { Clock, MapPin, Banknote, Lightbulb, ChevronDown, ChevronUp } from 'lucide-react';
...
const ActivityCard: React.FC<ActivityCardProps> = ({ 
  ...
  return (
    <div className={`bg-white border rounded-xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer ${
      ...
      {/* Activity Title */}
      <h3 className="font-bold text-gray-900 text-base leading-tight mb-2">{activity.title}</h3>

      {/* Activity Description */}
      <p className="text-gray-700 leading-relaxed text-sm mb-3">{activity.description}</p>
      
      {/* Location */}
      <div className="flex items-center space-x-2 bg-gray-50 px-3 py-2 rounded-lg mb-4">
        <a
